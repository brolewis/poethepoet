...
class ZshHarnessResult:
    ...
    @property
    def poe_task_args_output(self) -> str:
        """The raw output from poe _zsh_task_args (for debugging)."""
        return self._read_file("poe_task_args_output")
...
class ZshHarnessBuilder:
    ...
    def build_stubs(self) -> list[str]:
        ...
        lines = [
            ...
            "    # NOTE: Real _arguments -C sets state AND offers completions",
            "    # Check if previous word is -h/--help for help_task state",
...
"""
        ...
        # Capture poe _zsh_task_args output for debugging
        poe_output_capture = """
            # Harness: capture poe output for debugging
            poe _zsh_task_args "$current_task" $target_path 2>/dev/null > "$_HARNESS_DIR/poe_task_args_output"
...
class ZshHarnessRunner:
    ...
    def run(
        ...
        output_dir = self.work_dir / "harness_output"
        ...
        harness_file = self.work_dir / "harness.zsh"
...
def task_zsh_harness(path: str):
    ...
    The file can optionally include special comments to configure the harness:
        ...
        # MOCK _zsh_task_args: --opt\\tstring\\thelp\\t_
    """
    import sys
